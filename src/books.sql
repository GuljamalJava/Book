...
alter table book add column  publisher_id  int references book(id);
alter table book add column  author_id int references book(id);
alter table book add column language_id int references  book(id);
...
select distinct a.country from authors a  group by country;

-- TODO 3. 2020-2023 жылдардын арасындагы китептер чыксын.
select  name,published_year from book where published_year between'01/01/2020'  and '01/01/2023';

-- TODO 4.Детектив китептер жана алардын аттары чыксын.
select b.Genre,b.name from book b where b.Genre = 'DETECTIVE';

-- TODO 5.Автордун аты-жону author деген бир колонкага чыксын.
select (authors.first_name,authors.last_name) as full_name from authors;

-- TODO 6.Германия жана Франциядан болгон авторлорду толук аты-жону менен сорттоп чыгарыныз.
select a.first_name,a.last_name,a.country from authors a where a.country in('Germany','France');

--TODO 7.Романдан башка жана баасы 500 дон кичине болгон китептердин аты, олкосу, чыккан жылы, баасы жанры чыксын..
select b.name,b.country,b.published_year,b.price,b.Genre from book b where b.Genre != 'ROMANCE' and b.price < 500 order by b.price asc;
--TODO 8.Бардык кыз авторлордун биринчи 3 ну чыгарыныз.
select  first_name, last_name, gender  from authors where gender = 'FEMALE' order by gender limit 3;

--TODO 9.Почтасы .com мн буткон, аты 4 тамгадан турган, кыз авторлорду чыгарыныз.

select  first_name, last_name, gender,email  from authors where gender = 'FEMALE' order by length(first_name) > 4 and email = '.com%';
...
select  country, count(*) as count_authors from authors group by country having count(*) = 3  ;

--TODO 12. Ар бир жанрдагы китептердин жалпы суммасын чыгарыныз
select  Genre, sum(id) as book_id from book group by Genre;

--TODO 13. Роман жана Детектив китептеринин эн арзан бааларын чыгарыныз
select Genre , min(price) from book where Genre in ('DETECTIVE','ROMANCE')
group by Genre;

--TODO 14. История жана Биографиялык китептердин сандарын чыгарыныз
select Genre, count(id) as book_id from book where Genre in ('HISTORY','BIOGRAPHY') group by Genre;

--TODO 15. Китептердин , издательстволордун аттары жана тили чыксын
select  b.name, l.language,p.name from book b
                                           join languages l on b.language_id = l.id
                                           join publishers p on b.publisher_id = p.id group by b.name,l.language,p.name;

--TODO 16. Авторлордун бардык маалыматтары жана издательстволору чыксын, издательство болбосо null чыксын
select first_name,last_name,p.name from authors a
                                            left join book b on b.author_id = a.id
                                            left join publishers p on b.publisher_id = p.id group by  first_name,last_name,p.name;

--TODO 17. Авторлордун толук аты-жону жана китептери чыксын, китеби жок болсо null чыксын.
select a.first_name,a.last_name,b.name from authors a left join book b on a.id = b.author_id group by a.first_name,a.last_name,b.name;

--TODO 18. Кайсы тилде канча китеп бар экендиги ылдыйдан ойлдого сорттолуп чыксын.
select l.language,count(b.id)  as count_book from book b
                                                      join languages l on  b.language_id = l.id  group by l.language order by count_book desc;

--TODO 19. Издательствонун аттары жана алардын тапкан акчасынын орточо суммасы тегеректелип чыгарылсын.
select p.name,round(avg(b.price))  as avg_salary from publishers p
                                                          join public.book b on p.id = b.publisher_id group by p.name order by avg_salary;

--TODO 20. 2010-2015 жылдардын арасындагы китептер жана автордун аты-фамилиясы чыксын.
select published_year,b.name as book_name,a.first_name,a.last_name from book b
                                                                            join authors a on b.author_id = a.id  where published_year between  '01/01/2010' and  '01/01/2015' group by published_year,b.name,a.first_name,a.last_name;
...
select published_year,b.name as book_name,a.first_name,a.last_name,sum(b.price) as sum_salary from book b
                                                                                                       join authors a on b.author_id = a.id
where published_year between  '01/01/2010' and  '01/01/2015'
group by published_year,b.name,a.first_name,a.last_name order by sum_salary;
